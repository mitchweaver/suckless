...
void brelease(XEvent *e) {
	pid_t child;
    // for some reason this segafaults
	/* char cmd[100 + strlen(cwd)]; */
	/* char cmd[512]; // use this for now */
	...
	/* 			/1* sprintf(cmd, "(%s %s)", plumber_cmd, sel.primary); *1/ */
...
void xinit(void) {
    ...
    // if using alpha patch, comment this line:
	...
	xw.win = XCreateWindow(xw.dpy, parent, xw.l, xw.t,
			win.w, win.h, 0, DefaultDepth(xw.dpy, xw.scr), InputOutput,
...
void drawregion(int x1, int y1, int x2, int y2) {
	...
	if (!(win.state & WIN_VISIBLE)) return;
...
int main(int argc, char *argv[]) {
	...
	case 'g': xw.gm = XParseGeometry(EARGF(usage()), &xw.l, &xw.t, &cols, &rows); break;
