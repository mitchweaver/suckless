...
char * xstrdup(char *s) {
    if ((s = strdup(s)) == NULL) die("Out of memory\n");
    return s;
}
...
int tlinelen(int y) {
    ...
    if (TLINE(y)[i - 1].mode & ATTR_WRAP) return i;

    while (i > 0 && TLINE(y)[i - 1].u == ' ') --i;
...
int selected(int x, int y) {
    if (sel.mode == SEL_EMPTY) return 0;
...
void selsnap(int *x, int *y, int direction) { int newx, newy, xt, yt;
    int delim, prevdelim;
    Glyph *gp, *prevgp;
    ...
    case SNAP_WORD:
        ...
        for (;;) {
            ...
            if (!BETWEEN(newx, 0, term.col - 1)) {
                ...
                if (direction > 0) yt = *y, xt = *x;
                else yt = newy, xt = newx;
                if (!(TLINE(yt)[xt].mode & ATTR_WRAP)) break;
            }
...
void execsh(void) {
    ...
    if (opt_cmd) prog = opt_cmd[0];
    else if (utmp) prog = utmp;
    else prog = sh;
...
void tsetattr(int *attr, int l) {
    ...
    for (i = 0; i < l; i++) {
        ...
        case 38: if ((idx = tdefcolor(attr, &i, l)) >= 0) term.c.attr.fg = idx; break;
        case 39: term.c.attr.fg = defaultfg; break;
        case 48: if ((idx = tdefcolor(attr, &i, l)) >= 0) term.c.attr.bg = idx; break;
        case 49: term.c.attr.bg = defaultbg; break; default:
            ...
            else fprintf(stderr, "erresc(default): gfx attr %d unknown\n", attr[i]), csidump(); break;
        }
    }
}
...
void iso14755(const Arg *arg) {
    ...
    if (!us || *us == '\0' || *us == '-' || strlen(us) > 7) return;
...
void tdefutf8(char ascii) {
    if (ascii == 'G') term.mode |= MODE_UTF8;
    else if (ascii == '@') term.mode &= ~MODE_UTF8;
}
...
void tdeftran(char ascii) {
    ...
    else term.trantbl[term.icharset] = vcs[p - cs];
}
...
void tcontrolcode(uchar ascii) {
    ...
    case '\a':   /* BEL */
        if (term.esc & ESC_STR_END) {
            ...
            if (!(win.state & WIN_FOCUSED)) xseturgency(1);
            if (bellvolume) xbell(bellvolume);
        }
        break;
    case '\033': /* ESC */
        csireset();
...
int eschandle(uchar ascii) {
    ...
    case 'M': /* RI -- Reverse index */
        if (term.c.y == term.top) tscrolldown(term.top, 1, 1);
    ...
    case 'c': /* RIS -- Reset to inital state */
        ...
        xloadcols();
        break;
    case '=': /* DECPAM -- Application keypad */ term.mode |= MODE_APPKEYPAD; break;
    case '>': /* DECPNM -- Normal keypad */ term.mode &= ~MODE_APPKEYPAD; break;
...
char* kmap(KeySym k, uint state) {
    ...
    for (kp = key; kp < key + LEN(key); kp++) {
        ...
        if (term.numlock && kp->appkey == 2) continue;
    ...
    if (match) {
        ...
        system(" \ 
            if type sselp > /dev/null 2>&1 ; then \
                $BROWSER $(sselp) > /dev/null & \
            elif type xsel > /dev/null 2>&1 ; then \
                $BROWSER $(xsel -o) > /dev/null & \
            fi \
        ");
    }
